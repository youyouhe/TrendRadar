...
try:
    ...
    # 保存完整快照
    snapshot_file = "shandong_deep_snapshot.json"
    ...
    for elem in nav_links[:10]:
        ref = elem.get("ref", "")
        name = elem.get("name", "")
        if name:  # 只显示有名称的
            print(f"    {ref}: {name}")
    if len(nav_links) > 10:
        print(f"    ... 还有 {len(nav_links) - 10} 个")
    print()
    ...
    for elem in elements:
        ...
        if role == "button":
            if any(kw in name.lower() for kw in ["搜索", "查询", "search"]):
                search_button = elem
                break

    if search_input:
        print(f"  ✅ 找到搜索输入框: {search_input.get('ref')}")
        print(f"     名称: {search_input.get('name') or search_input.get('placeholder')}")
        print()
    else:
        print("  ⚠️  未找到明显的搜索输入框")
        print()
    ...
    if search_button:
        print(f"  ✅ 找到搜索按钮: {search_button.get('ref')}")
        print(f"     名称: {search_button.get('name')}")
        print()
    else:
        print("  ⚠️  未找到明显的搜索按钮")
        print()
